...
use crate::api::{Album, Artist, ArtistDetail, Playlist, PlaylistDetail, Song, SoundQuality, UserCounts, UserDetails, model::{AlbumDetail, LyricDetail, UserInfo}};
...
pub async fn get_artist_detail(id: u64) -> anyhow::Result<ArtistDetail> {
    ...
    match client().artist_detail(&query).await {
        Ok(resp) => {
            // let user = resp.body["data"]["user"].as_object().unwrap();
            let artist = resp.body["data"]["artist"].as_object().unwrap();
            Ok(ArtistDetail {
                id: artist["id"].as_u64().unwrap_or(0),
                name: artist["name"].as_str().unwrap_or("").to_string(),
                avatar: artist["avatar"].as_str().unwrap_or("").to_string(),
