...
[00:05.00]第二行"
        ...
        let lrc_lines: Vec<&LyricLine> =
            lines.iter().filter(|l| matches!(l.kind, LyricLineKind::Plain)).collect();
    ...
    fn test_inject_translations_skips_metadata_in_tl() {
        let mut lines = parse_lrc("[00:01.00]Hello\n[00:03.00]World");
        inject_translations(&mut lines, "[by:translator]\n[00:01.00]你好\n[00:03.00]世界");
    ...
    mod real_songs {
        ...
        fn parade_jp_lrc_cn_tlyric() {
            ...
            let expected: &[(&str, &str)] = &[
                ...
                ("身体の奥　喉の中で　言葉が出来る瞬間を僕は知りたいから", "但我还是想弄明白 在身体深处、喉咙之中 孕育出言语的那个瞬间啊"),
                ("このまま夜が明けたら", "若就这样迎来黎明"),
            ...
            for (i, &(jp_text, cn_text)) in expected.iter().enumerate() {
                ...
                assert_eq!(lines[i].translation.as_deref(), Some(cn_text), "line {} translation mismatch", i);
            }
        ...
        fn natsuzora_jp_lrc_cn_tlyric() {
            ...
            let checks: &[(usize, &str, &str)] = &[
                (0, "夜焦りの暗がり 去りし夏の宵(よい)", "焦躁夜裹的暗处中夏日浅夜经已过去"),
                (1, "二人眺めてるの", "俩人一同眺望着"),
                (2, "闇空に放たれるほど儚く", "仿似要绽放于夜空中般的虚幻"),
                (3, "降らせ今この静(せい)に 灯りはまだ遠く", "如今却散落的这份静寂灯火仍在远方"),
                (9, "夏が僕らを染め上げる光になって", "夏日化作将我们染上色彩的光芒"),
            ];
        ...
        fn kuusou_jp_lrc_cn_tlyric() {
            ...
            for i in 0..6 {
                assert!(lines[i].translation.is_some(), "line {} should have translation", i);
            }
            assert_eq!(lines[0].translation.as_deref(), Some("为了合乎情理活下去之类的"));
        ...
        fn u_song_lrc_cn_tlyric() {
            ...
            // 逐行验证翻译全匹配
            assert_eq!(lines[0].translation.as_deref(), Some("在注意到那夏日气息之前"));
            assert_eq!(lines[1].translation.as_deref(), Some("在蓬勃暑气快要焦熔化去之前"));
            assert_eq!(lines[2].translation.as_deref(), Some("再稍稍这样一会就好"));
            assert_eq!(lines[3].translation.as_deref(), Some("让我做一场不醒的美梦吧"));
        ...
        fn kaisei_jp_lrc_cn_tlyric() {
            ...
            assert_eq!(lines[0].translation.as_deref(), Some("那是时光尽头的 剧场世界的序章"));
        ...
        fn dakara_boku_full_translation_many_lines() {
            ...
            let pairs: &[(&str, &str)] = &[
                ...
                ("風が吹いた正午、昼下がりを抜け出す想像", "吹着风的正午 午后的思绪逐渐飘离"),
                ("ねぇ、これからどうなるんだろうね", "呐，今后该如何是好呢"),
                ("進め方教わらないんだよ", "向前迈进的方法没有学过啊"),
                ("君の目を見た\u{3000}何も言えず僕は歩いた", "看着你的双眼 什么也没说的就走了"),
                ("考えたってわからないし", "想过之后依然搞不懂"),
            ...
            for (i, &(jp_text, cn_text)) in pairs.iter().enumerate() {
                ...
                assert_eq!(lines[i].translation.as_deref(), Some(cn_text), "line {}: tl mismatch", i);
            }
        }
        ...
        fn pure_music_returns_none() {
            let detail = LyricDetail {
                lyric: None, tlyric: None, yrc: None, ytlrc: None,
                is_pure_music: true,
            };
    ...
    fn many_lines_stress() {
        ...
        for i in 0..200 {
            ...
            let sec = ms / 1000;
            let cs = (ms % 1000) / 10;
            lyric_lines.push_str(&format!("[{:02}:{:02}.{:02}]Line {}\n", sec / 60, sec % 60, cs, i));
            tl_lines.push_str(&format!("[{:02}:{:02}.{:02}]翻译{}\n", sec / 60, sec % 60, cs, i));
        }
